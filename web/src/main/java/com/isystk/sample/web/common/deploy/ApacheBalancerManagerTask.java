...
public class ApacheBalancerManagerTask {
    ...
    /**
     * @param args args[0] --- バランサマネージャURL args[1] --- クラスタ args[2] ---
     ...
     * @throws Exception
     */
    private static void switchBalancer(List<BalancerManagerConfig> balancers, String workerName, boolean enable) throws Exception {
	for (BalancerManagerConfig balancer : balancers) {
		...
		ret = ApacheBalancerManagerUtil.changeWorkerStatus(balancer.managerUrl, balancer.balancerName, workerName, enable);
		if (!ret) {
		    if (i > 0) {
