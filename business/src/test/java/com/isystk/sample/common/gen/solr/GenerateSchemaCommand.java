...
public class GenerateSchemaCommand {
	...
	Node n = e.getNextSibling();
	while (n != null
		&& (n.getNodeType() != Node.ELEMENT_NODE || (!n.getNodeName().equalsIgnoreCase(tagname1) && !n.getNodeName().equalsIgnoreCase(tagname2)))) {
	    n = n.getNextSibling();
	}
	return (Element) n;
    }
...
//        LOG.info(url);
	...
	if ("string".equals(type) || "text_ja".equals(type) || "text_ws".equals(type) || "text_ws2".equals(type)) {
	    ...
	    return "Date";
	} else if ("double".equals(type) || "tdouble".equals(type) || "pdouble".equals(type) || "sdouble".equals(type)) {
	    return "BigDecimal";
