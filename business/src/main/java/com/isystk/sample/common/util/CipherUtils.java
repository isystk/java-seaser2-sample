...
public final class CipherUtils {
    ...
    /** デフォルトログインキー */
    private static final String KEY_ORG = "^yBt7%jK]!7h{T@&";

    /** 暗号化方式 */
    private static final String CIPHER_RULE = ALGORITHM + "/CBC/PKCS5Padding";

    /** 接頭辞 */
    private static final String AGENT_LOGIN_KEY = "I6mwFu";
	...
	try {
	    ...
	    return KEY_ORG.substring(0, length);
	}
    }
	...
	String onetimekey = prefix + (new SimpleDateFormat(KEY_FORMAT)).format(now.getTime());
	...
	for (byte b : target) {
	    buider.append(String.format("%1$02x", (b & 0xff)));
	}
	...
	try {
		...
		logger.warn("復号化対象文字列が存在しません[" + org + "]");
		return new byte[0];
	    } else if (org.length() < 16) {
		throw new SystemException("復号化対象文字列が短いです[" + org + "]");
	    }
	    byte[] iv = getOriginalByte(org.substring(0, 16));
	    byte[] by = getOriginalByte(org.substring(16));
	    IvParameterSpec dps = new IvParameterSpec(iv);
	    Cipher cipher = Cipher.getInstance(CIPHER_RULE);
	    cipher.init(DECRYPT_MODE, new SecretKeySpec(key, ALGORITHM), dps);
	    return cipher.doFinal(by);
	} catch (Exception e) {
	    logger.warn("復号化失敗-->復号化前[" + org + "], key[" + new String(key) + "]");
	...
	for (int i = 0; i < ivLen; i++) {
	    int index = i * 2;
	    orgByte[i] = (byte) Integer.parseInt(org.substring(index, index + 2), 16);
	}
	return orgByte;
    }
	...
	try {
	    ois = new ObjectInputStream(new BufferedInputStream(new ByteArrayInputStream(CipherUtils.decodeByte(blowfish))));
	...
	try {
	    ...
	    oos.flush();
	    return CipherUtils.encode(baos.toByteArray());
	} catch (IOException e) {
	    throw new SystemException(e);
	} finally {
	    if (oos != null) {
	...
	String encoded3 = encode("");
	String encoded4 = encode("");
