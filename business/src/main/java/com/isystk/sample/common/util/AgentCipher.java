...
public final class AgentCipher {
	...
	private static byte[] getOriginalByte(String org) {
		...
		for (int i = 0; i < ivLen; i++) {
			int index = i * 2;
			orgByte[i] = (byte) Integer.parseInt(StringUtils.substr(org, index, index + 2), 16);
		}
		return orgByte;
	}
	...
	public static Object decodeObject(String blowfish) {
		...
		try {
			ois = new ObjectInputStream(
					new BufferedInputStream(new ByteArrayInputStream(AgentCipher.decodeByte(blowfish))));
